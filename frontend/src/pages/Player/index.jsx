            ...
            <div className="live-card">
              <div className="live-header">
                ...
                <button
                  ...
                  onClick={() => navigate(`/game/${live_game.gameId}`)}
                >
                  Voir & Parier →
                </button>
              </div>
              <div className="live-body">
                <div className="live-score-block">
                  ...
                  <div className="live-score" style={{ color: '#378add' }}>
                    {live_game.participants?.filter(p => p.teamId === 100).reduce((a, p) => a + (p.kills || 0), 0) || 0}
                  </div>
                </div>
                <div className="live-vs">vs</div>
                <div className="live-score-block" style={{ textAlign: 'right' }}>
                  <div className="live-side">Red side</div>
                  <div className="live-score" style={{ color: '#ef4444' }}>
                    {live_game.participants?.filter(p => p.teamId === 200).reduce((a, p) => a + (p.kills || 0), 0) || 0}
                  </div>
                </div>
                <div className="live-divider" />
                <div className="live-player-stats">
                  {(() => {
                    const p = live_game.participants?.find(p => p.puuid === player.riot_puuid || p.summonerName === player.summoner_name)
                    return p ? (
                      <>
                        <strong style={{ color: '#e8eaf0' }}>{p.championName}</strong><br />
                        {p.kills}/{p.deaths}/{p.assists} · {p.totalMinionsKilled} CS
                      </>
