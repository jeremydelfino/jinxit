...
# Les routes link-riot sont conservées mais plus exposées en UI
# (la liaison se fait à l'inscription — ces routes restent pour usage admin/futur)
...
async def link_riot_verify(
    ...
    if current_icon != current_user.riot_verification_icon:
        raise HTTPException(
            400,
            f"Mauvaise icône (actuelle : {current_icon}, attendue : {current_user.riot_verification_icon})",
        )
